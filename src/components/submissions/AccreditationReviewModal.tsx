...
export const AccreditationReviewModal: React.FC<AccreditationReviewModalProps> = ({
  ...
  const handleOpenPdf = async (docType: AccreditationDocType, storagePath: string) => {
    try {
      ...
      setActionError('Could not open PDF file. The file may have been moved or removed.');
    } finally {
  ...
  const handleDisapproveFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    <>
      <Modal show={show} onHide={onHide} size="xl" centered className="accreditation-review-modal">
        ...
        <Modal.Body className="p-4">
          {actionSuccess && (
            <Alert variant="success" className="d-flex align-items-center gap-2 mb-3 shadow-xs">
          ...
          {actionError && (
            <Alert variant="danger" className="d-flex align-items-center gap-2 mb-3 shadow-xs">
              ...
              <Card className="border shadow-sm h-100">
                ...
                <Card.Body className="p-3">
                  ...
                  {activeActionTab === 'disapprove' && (
                    <Form onSubmit={handleDisapproveFormSubmit}>
                      ...
                      <div className="mt-4 pt-2 border-top d-flex justify-content-end">
                        ...
                        >
                          ...
                          <span>Disapprove & Purge Files</span>
                        </Button>
                      </div>
                    </Form>
                  )}
      ...
      {/* Structured Confirmation Modal replacing in-browser confirm dialog */}
      <ConfirmDialog
        show={showDisapproveConfirm}
        onCancel={() => !loadingAction && setShowDisapproveConfirm(false)}
        onConfirm={handleConfirmDisapprove}
        title="Disapprove Accreditation Application"
        message={
          <>
            Are you sure you want to disapprove the accreditation application for <strong>{application.orgName}</strong>?
          </>
        }
        detail={
          <>
            <div className="fw-bold mb-1 text-dark">{application.orgName}</div>
            <div className="text-muted small mb-2">{application.contactEmail} · {application.barangay}</div>
            <div className="border-top pt-2 mt-1 small">
              <span className="text-muted fw-semibold">Stated Grounds: </span>
              <span className="text-dark">{disapproveReason}</span>
            </div>
          </>
        }
        warning="This action will notify the organization and permanently delete all 5 uploaded PDF documents from cloud storage."
        confirmLabel="Disapprove & Purge Files"
