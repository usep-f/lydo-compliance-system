 ...
 */
export const purgeSystemData = onCall(async (request) => {
  // 1. Authorization
  if (!request.auth || !request.auth.token || request.auth.token.role !== 'admin') {
    throw new HttpsError('permission-denied', 'Only administrators can perform a system purge.');
  }
  ...
  try {
    ...
    throw new HttpsError('internal', `Failed to purge system data: ${err.message}`);
  }
});
